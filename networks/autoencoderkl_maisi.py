...
class MaisiGroupNorm3D(nn.GroupNorm):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if len(input.shape) != 4:  # Changed from 5 to 4
            raise ValueError("Expected a 4D tensor")

        param_n, param_c, param_h, param_w = input.shape  # Removed param_d
        input = input.view(param_n, self.num_groups, param_c // self.num_groups, param_h, param_w)  # Removed param_d

        inputs = []
        for i in range(input.size(1)):
            array = input[:, i : i + 1, ...].to(dtype=torch.float32)
            mean = array.mean([2, 3, 4], keepdim=True)  # Removed 5th dimension
            std = array.var([2, 3, 4], unbiased=False, keepdim=True).add_(self.eps).sqrt_()  # Removed 5th dimension
        ...
        input = input.view(param_n, param_c, param_h, param_w)  # Removed param_d
        if self.affine:
            input.mul_(self.weight.view(1, param_c, 1, 1)).add_(self.bias.view(1, param_c, 1, 1))  # Removed 5th dimension
...
class MaisiConvolution(nn.Module):
    ...
    def _split_tensor(self, x: torch.Tensor, split_size: int, padding: int) -> list[torch.Tensor]:
        ...
        slices = [slice(None)] * 4  # Changed from 5 to 4 
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        padding_s = padding
        non_dim_split = self.dim_split + 1 if self.dim_split < 1 else 0  # Changed from < 2 to < 1
...
class MaisiUpsample(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = F.interpolate(x, scale_factor=2.0, mode="bilinear")  # Chnaged from trilinear to bilinear
        _empty_cuda_cache(self.save_mem)
        x = self.conv(x)
        _empty_cuda_cache(self.save_mem)
